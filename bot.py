...
async def render_full_page(url: str) -> tuple:
    ...
    Returns (final_html, network_asset_urls) where network_asset_urls is
    every resource URL the browser actually fetched while loading the page
    (covers JS-injected <img>, fetch()-loaded background images, fonts,
    XHR-loaded CSS, etc. that static HTML parsing alone would miss).
    """
    network_urls = set()
...
async def crawl_full_site(start_url: str, progress_cb=None) -> tuple:
    ...
    async def download_one(session_url, category, local_path):
        try:
            r = requests.get(session_url, headers=HEADERS, timeout=10)
            if r.status_code != 200:
                return
            content = r.content
            if category == "css":
...
async def handle_zip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    network_assets = set()
    try:
        html, network_assets = await render_full_page(url)
    except Exception as e:
    ...
    try:
        zip_buffer, asset_count = build_offline_zip(html, url, extra_asset_urls=network_assets)
    except Exception as e:
        await status_msg.edit_text(
...
async def handle_fullsite(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await log_activity(
        context,
        user,
        action="🚀 Full Website Crawl",
        detail=url,
        result=f"Pages: {page_count} | Assets: {asset_count} | Size: {size_mb} MB",
    )
